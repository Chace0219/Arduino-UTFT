...
{
    data = recvString(target, timeout);
    if (data.indexOf(target) != -1) {
        int32_t index1 = data.indexOf(begin);
		int32_t index2 = data.indexOf(end, index1 + begin.length() + 1);
		if (index1 != -1 && index2 != -1) {
            index1 += begin.length();
			if (index2 > 500)
			{
				data.remove(index2);
				data.remove(0, index1);
			}
			else
			{
...
bool ESP8266::eATCIPSTATUS(String &list)
{
    ...
    return recvFindAndFilter("OK", "STATUS:", "\r\n", list);
}
...
bool ESP8266::eATCIFSR(String &list)
{
    ...
    return recvFindAndFilter("OK", "STAIP,\"", "\"", list);
	//return recvFindAndFilter("OK", "\r\r\n", "\r\n\r\nOK", list);
}

bool ESP8266::eATCIPSTAMAC(String &list)
{
	rx_empty();
	m_puart->println("AT+CIPSTAMAC_CUR?");
	return recvFindAndFilter("OK", "CIPSTAMAC_CUR:\"", "\"", list);
}
