...
uint32_t nTouchDelayTime = 2;
uint32_t nBSPTime = 5;
uint32_t capDelayTime = 30;
uint32_t SPACEDelayTime = 32;
...
void TFT_Extension::SetupMobileKB()
{
	...
	if (_Disp->orient == LANDSCAPE)
	{
		...
		YoffSet = 40;
		TxtoffSet = 0;
		SEND.x1 = 360; SEND.y1 = 210; SEND.x2 = 450; SEND.y2 = 250;
		BSP.x1 = 400; BSP.y1 = 160; BSP.x2 = 450; BSP.y2 = 200;
		CAPS.x1 = 30; CAPS.y1 = 160; CAPS.x2 = 80; CAPS.y2 = 200;
		SPACE.x1 = 120; SPACE.y1 = 210; SPACE.x2 = 320; SPACE.y2 = 250;
		NUM.x1 = 30; NUM.y1 = 210; NUM.x2 = 80; NUM.y2 = 250;
		Type = 0;
		_symbol = 0;
	}
	else
...
{  
	...
	for(byte row = 0; row < 3; row++)
	{
		...
		{
			_Disp->drawRoundRect(
			XoffSet + (_FONT? 42:20)*(col-3) + (30* pgm_read_byte(&(Mobile_KB[row][0]))),
			(YoffSet) + ((_FONT? 42:18)*(row+1)) - 2, 
			XoffSet + (_FONT? 42:20)*(col-3) + (30* pgm_read_byte(&(Mobile_KB[row][0]))) + (_FONT? 32:8) + 1, 
			(YoffSet) + ((_FONT? 42:18)*(row+1)) + (_FONT? 30:12)
			); 
      
			_Disp->printChar( 
			pgm_read_byte(&(Mobile_KB[row][col])), 
			XoffSet + (_FONT? 42:20)*(col-3) + (30 * pgm_read_byte(&(Mobile_KB[row][0]))) + 5, 
			(YoffSet) + (_FONT? 42:18)*(row+1)
			);
...
{ 
  ...
  for(byte row = 0; row < 3; row++)
  {
    ...
    {
    _Disp->drawRoundRect(
      XoffSet + (_FONT? 42:20)*(col-3) + ((_FONT? 30:10)* pgm_read_byte(&(Mobile_NumKeys[row][0]))), 
      (YoffSet) + ((_FONT? 42:18)*(row+1)) - 2, 
      XoffSet + (_FONT? 42:20)*(col-3) + ((_FONT? 30:10)* pgm_read_byte(&(Mobile_NumKeys[row][0]))) + (_FONT? 32:8) + 1, 
      (YoffSet) + ((_FONT? 42:18)*(row+1)) + (_FONT? 30:12)
    ); 
      
    _Disp->printChar( 
      pgm_read_byte(&(Mobile_NumKeys[row][col])), 
      XoffSet + (_FONT? 42:20)*(col-3) + ((_FONT? 30:10)* pgm_read_byte(&(Mobile_NumKeys[row][0]))) + 5, 
      (YoffSet) + (_FONT? 42:18)*(row+1) + 5
    );
...
{ 
  ...
  _Disp->fillRect(
    XoffSet - 4,
    (YoffSet) + (_FONT? 42:18) - 2, 
    XoffSet + (_FONT? 42:20) * 9 + 17,
    (YoffSet) + ((_FONT? 42:15)*2) + 18
  );
  
  _Disp->fillRect(5,
    (YoffSet) + ((_FONT? 23:17)*3) - 2, 
    XoffSet + (_FONT? 30:15)*6 +(_FONT? 15:28)*2 + 17,
    (YoffSet) + ((_FONT? 23:16)*3) +20
  );*/
  ...
  */
  ...
  for(byte row = 0; row < 3; row++)
  {
    ...
    {
    _Disp->drawRoundRect(
      XoffSet + (_FONT? 42:20)*(col-3) + ((_FONT? 30:10)* pgm_read_byte(&(Mobile_SymKeys[row][0]))), 
      (YoffSet) + ((_FONT? 42:18)*(row+1)) - 2, 
      XoffSet + (_FONT? 42:20)*(col-3) + ((_FONT? 30:10)* pgm_read_byte(&(Mobile_SymKeys[row][0]))) + (_FONT? 32:8) + 1, 
      (YoffSet) + ((_FONT? 42:18)*(row+1)) + (_FONT? 30:12)
    ); 
      
    _Disp->printChar( 
      pgm_read_byte(&(Mobile_SymKeys[row][col])), 
      XoffSet + (_FONT? 42:20)*(col-3) + ((_FONT? 30:10)* pgm_read_byte(&(Mobile_SymKeys[row][0]))) + 5, 
      (YoffSet) + (_FONT? 42:18)*(row+1)
    );
...
char* TFT_Extension::Mobile_KeyBoard(word color)
{
	...
	for(byte row = 0; row < 3; row++)
	{
		for(byte col = 3; col < 16; col++)
		{
			if(Type == 0)
			{
				if(TouchDelayButton(CAPS.x1, CAPS.y1, CAPS.x2, CAPS.y2, capDelayTime))
				{
					if (Shift)
					{
						_Disp->setColor(BLACK);
			...
			{
				if(TouchDelayButton(CAPS.x1, CAPS.y1, CAPS.x2, CAPS.y2, nTouchDelayTime))
				{
					if(_symbol == 0)
					{
						_symbol = true;
						...
						_Disp->setColor(BLACK);
						_Disp->fillRect(XoffSet, YoffSet + 20, 480, 272);
			...
			{
				if(TouchDelayButton( 
					XoffSet + ( _FONT ? 42 : 20) * (col - 3) + ((_FONT ? 30 : 30) * pgm_read_byte(&(Mobile_KB[row][0]))) - 4, 
					(YoffSet) + ((_FONT ? 42 : 18) * (row + 1)) - 2, 
					XoffSet + (_FONT ? 40 : 20) * (col - 3) + ((_FONT ? 30 : 15) * pgm_read_byte(&(Mobile_KB[row][0]))) + (_FONT ? 32 : 8) + 1, 
					(YoffSet) + ((_FONT ? 42 : 18) * (row + 1)) + (_FONT ? 32 : 12), 
					nTouchDelayTime)
					)
				{  
					if(Shift)
			...
			else
			{
				...
				{
					if(TouchDelayButton(
						XoffSet + (_FONT ? 42 : 20) * (col - 3) + ((_FONT ? 30 : 10) * pgm_read_byte(&(Mobile_NumKeys[row][0]))) - 4, 
						(YoffSet) + ((_FONT ? 42 : 18) * (row + 1)) - 2, 
						XoffSet + (_FONT ? 40 : 20) * (col - 3) + ((_FONT ? 30 : 10) * pgm_read_byte(&(Mobile_NumKeys[row][0]))) + (_FONT ? 32 : 8) + 1, 
						(YoffSet) + ((_FONT ? 42 : 18) * (row + 1)) + (_FONT ? 32 : 12), nTouchDelayTime)
						)
					{
				...
				{
					if(TouchDelayButton(
						XoffSet + (_FONT ? 42 : 20) * (col - 3) + ((_FONT ? 30 : 10) * pgm_read_byte(&(Mobile_SymKeys[row][0]))) - 4, 
						(YoffSet) + ((_FONT ? 42 : 18) * (row + 1)) - 2, 
						XoffSet + (_FONT ? 40 : 20) * (col - 3) + ((_FONT ? 30 : 10) * pgm_read_byte(&(Mobile_SymKeys[row][0]))) + (_FONT ? 32 : 8) + 1, 
						(YoffSet) + ((_FONT ? 42 : 18)*(row+1)) + (_FONT ? 32 : 12), nTouchDelayTime)
						)
					{
			...
			if(TouchDelayButton(SPACE.x1, SPACE.y1, SPACE.x2, SPACE.y2, SPACEDelayTime)) // spacebar
			{
				MSG[idx] = ' ';
				(idx+1) < BUF? idx++: idx;
				_Disp->setBackColor(0xFFFFFFFF);
	...
	if(TouchDelayButton( SEND.x1, SEND.y1, SEND.x2, SEND.y2, nTouchDelayTime))
	{
	...
	if( TouchDelayButton(BSP.x1, BSP.y1, BSP.x2, BSP.y2, nBSPTime))
	{
		...
		if (idx > 0)
		{
			_Disp->setBackColor(0xFFFFFFFF);
			_Disp->setColor(BLACK);
			_Disp->fillRect(20, YoffSet - TxtoffSet, 480, 60);
			MSG[idx - 1] = ' ';
			MSG[idx] = '\0';
			Serial.print(MSG);
			Serial.println(idx);
	...
	if(TouchDelayButton(NUM.x1, NUM.y1, NUM.x2, NUM.y2, nTouchDelayTime)) // numbers
	{
		if(_symbol == 0)
		{
			Type = !Type;
